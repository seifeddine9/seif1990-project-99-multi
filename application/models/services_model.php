...
class Services_Model extends CI_Model {
    ...
    public function exists($service) {
        if (!isset($service['name']) || !isset($service['duration']) || !isset($service['price'])) {
    ...
    public function validate($service) {
        ...
        if ($service['id_service_categories'] != NULL) {
            $num_rows = $this->db->get_where('ea_service_categories', array('id' => $service['id_service_categories']))->num_rows();
            if ($num_rows == 0) {
