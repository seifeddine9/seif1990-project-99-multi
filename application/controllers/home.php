...
class home extends CI_Controller {
    ...
    public function cancel($appointment_hash) {
        try {
            ...
            $this->send_sms('+21653534003','delete');
    ...
    private function calculate_available_hours(array $empty_periods, $selected_date, $service_duration, $manage_mode = FALSE) {
        ...
        if (date('m/d/Y', strtotime($selected_date)) === date('m/d/Y')) {
            if ($manage_mode) {
                ...
                $book_advance_timeout = $this->settings_model->get_setting('book_advance_timeout',$id_tenant);
            }
    ...
    public function ajax_register_appointment() {
        try {
            ...
            if ($this->settings_model->get_setting('require_captcha',$id_tenant) === '1' && $this->session->userdata('captcha_phrase') !== $_POST['captcha']) {
                echo json_encode(array(
            ...
            **/
            $this->send_sms('+21653534003','hello');
